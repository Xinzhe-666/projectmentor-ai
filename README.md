...
| 项目问答 | MVP | V4.2-2 支持轻量检索增强问答、历史记录、建议追问、复制回答和证据展开，不接入向量数据库 |
...
- 项目问答仍是轻量 MVP：只基于已保存文件做关键词检索和证据片段抽取，不接入 Milvus、向量数据库或 embedding。
...
- 后续 V4.2-3 可再评估 embedding / 向量检索升级。
...
- V4.2：项目问答。V4.2-2 已完成轻量检索增强问答、问答历史、建议追问、复制回答和证据展开；V4.2-3 可再评估 embedding / 向量检索升级。
- V4.3：管理员后台 / 用户反馈。
- V4.4：更正式的部署和用户体系。
