...
| 审计报告 | 已完成 | 输出评分、风险点、建议和简历改写版本，已支持报告详情页浏览器打印 / 保存为 PDF |
...
| 面试深挖 | 基础版 | 规则版 V1，支持会话、追问、评分、总结，已支持面试复盘打印 / 保存为 PDF |
...
- ZIP 上传安全过滤：支持普通项目 ZIP，最大 200MB，自动过滤 `.git`、`target`、`node_modules`、`dist`、`build` 等目录，只解析核心文本文件。
...
- V4.0：当前可试用版本。
- V4.1：PDF 导出 / 分享报告（已支持报告页和面试复盘浏览器打印 / 保存为 PDF，已支持审计报告只读分享链接；正式后端 PDF 导出可作为后续增强）。
- V4.2：RAG 项目问答。
- V4.3：管理员后台 / 用户反馈。
- V4.4：更正式的部署和用户体系。
