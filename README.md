...
| 管理员后台 | MVP | V4.3-1 新增只读数据看板，通过 `ADMIN_EMAILS` 配置管理员邮箱，不引入复杂 RBAC |
...
- 管理员后台先做只读 MVP：通过环境变量配置管理员邮箱，只查看用户、项目、报告、分享和问答统计，不返回密码、密钥或项目源码内容。
...
- 管理员后台当前是只读 MVP，不做复杂 RBAC，不提供用户删除、封号、改额度或数据导出能力。
...
- V4.3：管理员后台 / 用户反馈。V4.3-1 已完成管理员只读数据看板，后续可继续补充反馈管理。
- V4.4：更正式的部署和用户体系。
