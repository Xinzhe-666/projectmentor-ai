...
- 将验证码校验与消费分离：校验成功不再提前删除验证码，只有注册成功后才一次性消费。
- 修复注册业务失败时验证码已被提前消费的问题，用户可修正注册信息后继续使用仍在有效期内的验证码。
- 修复 SMTP 发送失败后验证码和发送 cooldown 残留的问题；清理采用 best effort，不覆盖原始发送失败提示。
- SMTP 发送失败时保留 email / IP 小时发送计数，避免利用失败路径无限触发邮件服务。
- 增加注册验证码错误尝试次数限制，默认最多 5 次；达到上限后当前验证码立即失效。
- 成功发送新验证码后重置该邮箱各 IP 维度的旧校验失败次数；计数按验证码 generation 隔离，避免 Redis 短暂故障恢复后继承旧计数。
- Redis 正常时优先使用 Redis；Redis 不可用时保留进程内验证码与失败次数限制 fallback。

边界：

- 不改变数据库、credits、JWT、AI 或 SMTP Provider。
- 不做密码找回、密码修改、OAuth、验证码数据库表或消息队列。
...
- 优化注册页验证码输入和发送按钮体验，验证码输入框继续与发送按钮保持紧凑布局。
- 发送成功后提示用户查看邮箱或垃圾箱。
- 发送成功后按钮进入 60 秒倒计时，倒计时期间不可重复点击。
- 移动端验证码区域在窄屏下上下排列，不再挤压。
- 梳理验证码相关错误提示，覆盖邮箱已注册、发送频率、发送次数、验证码为空 / 过期 / 错误和发送失败等场景。
...
# V4.8-1 注册邮箱验证码线上验收与部署配置固化

本版本固化注册邮箱验证码上线后的部署配置和验收文档，不改变验证码业务逻辑、不改变 credits、不改数据库。

新增与修复：

- 线上 Foxmail / QQ SMTP 发码已验证成功，注册邮箱验证码已进入可上线验收状态。
- `backend` 容器通过 `env_file: .env` 读取服务器 `.env`，同时保留 Compose 中的核心环境变量配置。
- 修复部署时邮箱验证码相关环境变量未进入 backend 容器导致生产环境无法读取 SMTP 配置的问题。
- `.env.example` 补齐邮箱验证码配置示例，只保留占位值，不包含真实 SMTP 授权码或邮箱密码。
- README、Docker 部署文档和 VPS 部署文档补充邮箱验证码上线说明、环境变量检查命令和发码测试命令。
- 补充 2G 轻量服务器部署注意事项：不要在服务器执行 `docker compose up -d --build`、`npm run build` 或 `mvn clean package`；后端 jar 和前端 dist 推荐本地构建后上传。

边界：

- 不改变注册邮箱验证码业务逻辑。
- 不改变 credits 扣费、注册赠送额度、登录 JWT、管理员权限或 AI 调用逻辑。
- 不新增数据库表或字段。
- 不提交真实 `.env`、SMTP 授权码、邮箱密码、AI Key 或 `JWT_SECRET`。
