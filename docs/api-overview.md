...
| 方法 | 路径 | 用途 | 是否需要登录 |
| --- | --- | --- | --- |
| `POST` | `/api/auth/email-code` | 发送注册邮箱验证码；未登录可调用，受发送 cooldown 及 email / IP 小时限制；成功发送新验证码会重置该邮箱各 IP 维度的旧校验失败次数 | 否 |
| `POST` | `/api/auth/register` | 注册用户并返回 token；开启邮箱验证码后需要提交验证码，校验失败次数默认最多 5 次，只有注册成功后才一次性消费验证码；成功注册受同 IP 每小时 3 次、每天 10 次限制 | 否 |
| `POST` | `/api/auth/login` | 用户登录并返回 token | 否 |
| `GET` | `/api/auth/me` | 获取当前登录用户信息 | 是 |
| `POST` | `/api/auth/logout` | 退出登录；当前版本主要由前端清理 token | 是 |

注册邮箱验证码同时具备发送 cooldown、email / IP 小时发送限制和校验失败次数限制。校验失败计数按规范化邮箱、客户端 IP 与当前验证码 generation 隔离，避免新验证码继承旧计数；存在有效验证码时，验证码错误或不是 6 位数字都会计入失败次数。验证码校验通过后不会立即删除；注册业务成功后才一次性消费，注册业务失败时仍可在有效期内重试。达到最大校验失败次数后，当前验证码立即失效，需要重新获取。
