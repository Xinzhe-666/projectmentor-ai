...
| 管理员后台 | MVP | V4.3-2 支持只读数据看板和管理员手动发放额度，通过 `ADMIN_EMAILS` 配置管理员邮箱，不引入复杂 RBAC |
...
- 管理员后台保持克制边界：通过环境变量配置管理员邮箱，可查看统计和最近记录，也可为指定用户增加额度；额度发放必须写入流水，不返回密码、密钥或项目源码内容。
...
- `credits`：查看额度、额度流水。
- `admin`：管理员身份检查、系统统计和最近用户 / 项目 / 报告 / 问答只读列表。
...
- 管理员后台当前不做复杂 RBAC，不提供用户删除、封号、扣减额度或数据导出能力；V4.3-2 只支持管理员手动增加额度，不是支付系统。
...
- V4.3：管理员后台 / 用户反馈。V4.3-2 已完成管理员只读数据看板和额度发放，后续可继续补充反馈管理。
- V4.4：更正式的部署和用户体系。
